resource "aws_db_subnet_group" "rds_subnet_group" {
  name       = "rds-subnet-group-${var.tag_environment}"
...
resource "aws_db_instance" "rds" {
  ...
  engine                      = var.rds_engine         # Engine do PostgreSQL
  engine_version              = var.rds_engine_version # Versão do PostgreSQL
  instance_class              = var.rds_instance_type  # Instância elegível para o Free Tier
  db_name                     = var.rds_db_name        # Nome do banco de dados
  identifier                  = var.rds_identifier     # Identificador da instância RDS
  username                    = var.rds_username       # Nome de usuário do banco
  ...
  vpc_security_group_ids                = [aws_security_group.rds_db.id]          # Grupo de segurança
  db_subnet_group_name                  = aws_db_subnet_group.rds_subnet_group.id # Subnet do VPC
  backup_retention_period               = var.rds_backup_retention_period         # Período de retenção de backups automáticos
